...
# world over celebrating the industry of so many volunteers who are
...
population <- read_csv("population.csv", col_names = TRUE) %>% 
  select(`Country Name`, `2018`) %>% 
  rename(country=`Country Name`,population=`2018`)

population$country <- ifelse(population$country=="United States", "US", population$country)
                             
# use JH repo URL for this dat
...
s2 <-  s1 %>% pivot_longer(-c(province, country), 
                           names_to = "date",
                           # this maybe needs a format string to convert - see below
                           names_ptypes = c("date", date),
                           values_to = "cumulative_cases")

# this is time series convert from US coloquial date format
...
s3 <- s2 %>% arrange(date, country, province) %>% group_by(date, country)

s3a <- s3 %>% summarise(Total=sum(cumulative_cases)) %>% ungroup()

s3b <- s3a %>% arrange(country) %>% group_by(country)
...
s5 <- s4 %>% mutate(growth = safe_ratio(new_cases,Total), PerCapita = Total/population/100) 
...
s6 <- s5 %>% filter(country %in% countries, date > as.Date("2020-02-14"), Total > 100)
